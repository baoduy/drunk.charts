...
if [ $STATUS_CODE -eq 0 ]; then
    ...
    helm upgrade $RELEASE_NAME -f $VALUES_FILE $CHART_PATH --namespace $NAMESPACE
else
    echo "Release $RELEASE_NAME does not exist in the namespace $NAMESPACE. Installing..."
    # Install new release
    helm install $RELEASE_NAME -f $VALUES_FILE $CHART_PATH --create-namespace --namespace $NAMESPACE
fi