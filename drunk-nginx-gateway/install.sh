#!/bin/bash
# Install script for drunk-nginx-gateway chart
# Two-phase installation:
#  1. Install Gateway API CRDs via kubectl (bypasses Helm 3MB annotation limit)
#  2. Install Helm chart for GatewayClass, Gateway, HTTPRoute, NGINX Gateway Fabric
#
# Usage (basic):
...
#   GATEWAY_API_VERSION=v1.2.0 ./install.sh
...
echo ""
info "========================================="
info "drunk-nginx-gateway Installation"
info "========================================="
...
echo ""
info "Phase 2: Installing Helm chart (GatewayClass, Gateway, HTTPRoute, NGINX Gateway Fabric)"
...
NGF_ENABLED=$(grep -E '^nginxGatewayFabric:' -A3 "$CHART_DIR/$VALUES_FILE" | grep -E 'enabled:[[:space:]]*true' || true)

HELM_SKIP_CRDS_FLAG="--skip-crds"
if [[ -n "$NGF_ENABLED" ]]; then
...
echo "  kubectl get pods -n $NAMESPACE -l app.kubernetes.io/name=nginx-gateway-fabric"
echo ""
info "Describe resources:"
echo "  kubectl describe gatewayclass"
echo "  kubectl describe gateway -n $NAMESPACE"
echo ""
info "Inspect NGF custom CRDs:"
echo "  kubectl get crd | grep nginx.org"
echo ""
