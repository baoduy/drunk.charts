...
Verification: `drunk-lib/verify.sh` runs `helm package` + `helm repo index` and copies the latest `.tgz` into `drunk-app/charts/`. `drunk-app/verify.sh` validates the application chart build/render path. Per user instruction, after changing `drunk-lib/` run `bash drunk-lib/verify.sh`, and after changing `drunk-app/` run `bash drunk-app/verify.sh`.
...
- After changing anything under `drunk-app/`, run `bash drunk-app/verify.sh` before declaring done.
